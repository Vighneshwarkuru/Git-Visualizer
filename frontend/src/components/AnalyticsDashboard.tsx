          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            {analytics.mostModifiedFiles.map((fileStats, idx) => (
              <div key={fileStats.file} className="flex items-center gap-3">
                <div className="w-6 h-6 rounded-xl bg-slate-800 flex items-center justify-center text-[10px] text-slate-400 font-bold font-mono">
                  {idx + 1}
                </div>
            ...
            {contributors.map((contrib) => (
              <div key={contrib.email} className="flex items-center justify-between p-2.5 rounded-2xl bg-[#1d1b20]/50 hover:bg-slate-800/20 border border-transparent/40">
                ...
                <div className="text-right flex-shrink-0">
                  <span className="text-xs font-black font-mono text-[#d0bcff]">{contrib.commits}</span>
